...
`;
  ...
  function applyRotationState() {
    ...
    if (!coverArt && !fadArt) return false;

    const playing = Spicetify.Player.isPlaying();
    const value = playing ? "running" : "paused";
    coverArt?.style.setProperty("animation-play-state", value);
    fadArt?.style.setProperty("animation-play-state", value);
    isPlaying = playing;
    return true;
  }

  function handleRotate() {
    // Elemente werden von Spotify erst spaeter gerendert -> warten
    if (!applyRotationState()) {
      setTimeout(handleRotate, 250);
      return;
    }
    // Nachlauf: bei playpause ist der Player-State kurz nach dem Event noch alt
    setTimeout(applyRotationState, 150);
  }
  ...
  function handleTracksNamePreview() {
    ...
    // Ohne Vorgaenger/Nachfolger in der Queue sonst TypeError -> bricht die Initialisierung ab
    if (!prevTrack?.contextTrack || !nextTrack?.contextTrack || !currentTrack?.contextTrack) return;
